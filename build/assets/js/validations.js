...
function validateRecruiterRegForm() {
    ...
    // Defining error variables with a default value
    var firstNameErr = lastNameErr = emailErr = phoneErr = passwordErr = companyNameErr = industryErr = true;
    ...
    // Prevent the form from being submitted if there are any errors
    if ((firstNameErr || lastNameErr || emailErr || phoneErr || companyNameErr || industryErr || passwordErr) == true) {
        return false;
    } else {
        return true;
    }
...
function validatePostJobForm(job_title, job_type, job_category, location, industry, job_description, job_responsibilities, min_qualification, experience_level, min_year_of_experience, max_year_of_experience, expected_salary, gender_type, application_deadline, minimum_age, maximum_age, required_skills, shortlist_params) {
    ...
    if (ifConfirmed) {
        ...
        // Prevent the form from being submitted if there are any errors
        if ((jobTitleErr || jobTypeErr || jobCategoryErr || locationErr || industryErr || jobDescriptionErr || minQualificationErr || experienceLevelErr || minYearOfExperienceErr || minYearOfExperienceErr || genderTypeErr) == true) {
            console.log("false");
            return false;
        } else {
            console.log("true");
...
function validateCreateAssessmentForm(assessment_name, assessment_type, job_assigned_to, assessment_time, assessment_description) {
    ...
    if (ifConfirmed) {
        ...
        // Prevent the form from being submitted if there are any errors
        if ((assessmentNameErr || assessmentTypeErr || jobAssignedToErr || assessmentTimeErr || assessmentDescriptionErr) == true) {
            console.log("false");
            return false;
        } else {
            console.log("true");
...
function validateCreateInterviewForm() {
    ...
    if (ifConfirmed) {
        ...
        // Prevent the form from being submitted if there are any errors
        if ((interviewNameErr || interviewDateErr || interviewTimeErr || venueErr || dressCodeErr || jobAssignedToErr) == true) {
            return false;
        } else {
            return true;
        }
...
function validateUpdateCompanyProfileForm(company_name, rc_number, company_phone_number, company_email, industry, year_established, company_description, website, no_of_employees, address, city, state, country, twitter_link, instagram_link, facebook_link, linkedin_link) {
