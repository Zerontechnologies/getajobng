...
$(document).ready(function () {
        ...
        success: function (data) {
            ...
            if (!data) {
                ...
                for (var i = 0; i < data.industries.length; i++) {
                    industryContent += '<label for="" class="containerr" id="industry_label">' + data.industries[i].industry_name + '<input type="checkbox" ' + 'value="' + data.industries[i].industry_id + '" ' + 'name="' + data.industries[i].industry_name + '" ' + 'id="' + data.industries[i].industry_name + '"> \
                ...
                for (var i = 0; i < data.states.length; i++) {
                    stateContent += '<label for="" class="containerr">' + data.states[i].state_name + '<input type="checkbox" ' + 'value="' + data.states[i].state_id + '" ' + 'name="' + data.states[i].state_name + '" ' + 'id="' + data.states[i].state_name + '"> \
                                            <span class="checkmark"></span> \
                                        </label>';
                }
                ...
                for (var i = 0; i < data.jobTypes.length; i++) {
                    jobTypeContent += '<label for="" class="containerr">' + data.jobTypes[i].job_type_name + '<input type="checkbox" ' + 'value="' + data.jobTypes[i].job_type_id + '" ' + 'name="' + data.jobTypes[i].job_type_name + '" ' + 'id="' + data.jobTypes[i].job_type_name + '"> \
                                            <span class="checkmark"></span> \
                                        </label>';
                }
                ...
                for (var i = 0; i < data.skills.length; i++) {
                    skillContent += '<label for="" class="containerr">' + data.skills[i].skill_name + '<input type="checkbox" ' + 'value="' + data.skills[i].skill_id + '" ' + 'name="' + data.skills[i].skill_name + '" ' + 'id="' + data.skills[i].skill_name + '"> \
                                            <span class="checkmark"></span> \
                                        </label>';
                }
            }
                    ...
                    for (var i = 0; i < data.jobs.length; i++) {
                        content += '<tr> \
                                        <td> \
                                            <div class="site_row"> \
                                                <p> \
                                                    <a href="/job-detail/' + data.jobs[i].job_id + '">' + data.jobs[i].job_name + '</a>\
                                                </p>\
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (!data.industries) {
                ...
                for (var i = 0; i < data.industries.length; i++) {
                    industryContent += '<option value="' + data.industries[i].industry_id + '">' + data.industries[i].industry_name + '</option>';
                }
            }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (!data.states) {
                ...
                for (var i = 0; i < data.states.length; i++) {
                    stateContent += '<option value="' + data.states[i].state_id + '">' + data.states[i].state_name + '</option>';
                }
            }
            ...
            if (!data.qualifications) {
                ...
                for (var i = 0; i < data.qualifications.length; i++) {
                    qualificationContent += '<option value="' + data.qualifications[i].qualification_id + '">' + data.qualifications[i].qualification_name + '</option>';
                }
            }
            ...
            success: function (data) {
                ...
                if (!data || data.talents.length == 0) {
                    ...
                    for (var i = 0; i < data.talents.length; i++) {
                        ...
                        var picture = typeof data.talents[i].photo_url != 'undefined' && data.talents[i].photo_url != null && data.talents[i].photo_url != 'null' && data.talents[i].photo_url != '' ? data.talents[i].photo_url : '/images/no-user.png';

                        var profile_summary = typeof data.talents[i].profile_summary != 'undefined' && data.talents[i].profile_summary != 'null' && data.talents[i].profile_summary != null && data.talents[i].profile_summary != '' ? data.talents[i].profile_summary : 'No personal summary provided';

                        var tagline = typeof data.talents[i].tagline != 'undefined' && data.talents[i].tagline != null && data.talents[i].tagline != 'null' && data.talents[i].tagline != '' ? data.talents[i].tagline : '';
    ...
    ClassicEditor.create(document.querySelector('#company_description'), {
        toolbar: ['heading', '|', 'bold', 'italic', 'link', 'bulletedList', 'numberedList', 'blockQuote'],
        heading: {
            options: [{ model: 'paragraph', title: 'Paragraph', class: 'ck-heading_paragraph' }, { model: 'heading1', view: 'h1', title: 'Heading 1', class: 'ck-heading_heading1' }, { model: 'heading2', view: 'h2', title: 'Heading 2', class: 'ck-heading_heading2' }]
        }
    }).then(editor => {
    ...
    $("#update_company_profile_btn").click(function () {
        ...
        var ifValidated = validateUpdateCompanyProfileForm(company_name, rc_number, company_phone_number, company_email, industry, year_established, company_description, company_website, no_of_employees, address, city, state, country, twitter_link, instagram_link, facebook_link, linkedin_link);

        if (ifValidated) {
            updateCompanyProfileForm(company_name, rc_number, company_phone_number, company_email, industry, year_established, company_description, company_website, no_of_employees, address, city, state, country, twitter_link, instagram_link, facebook_link, linkedin_link);
        }
    });

    function updateCompanyProfileForm(company_name, rc_number, company_phone_number, company_email, industry, year_established, company_description, website, no_of_employees, address, city, state, country, twitter_link, instagram_link, facebook_link, linkedin_link) {
